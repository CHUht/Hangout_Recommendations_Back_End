...
def generate_user_recommendations(Events, user_ratings, user_id, Recommendations):
    """
   ...
   """
    ...
    for rating in user_ratings:
        ratings_with_events_list.append((id_to_index[rating[0]],rating[1]))
    ...
    for i in range(len(similarities_matrix)):
        score = 0
        for j, rating in ratings_with_events_list:
            score = score + (rating - 3) * similarities_matrix[i, j]
...
if __name__ == "__main__":
    ...
    ratings = Ratings.get_ratings_from_user(2)
    generate_user_recommendations(Events, ratings, 2, Recommendations)